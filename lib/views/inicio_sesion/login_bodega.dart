...
class LoginBodega extends StatefulWidget {
  ...
  final String? metodo;
  final int comprueba;
  const LoginBodega({this.codBodega, this.nombreBodega, this.longitud, this.latitud, required this.comprueba, this.metodo, Key? key})
      : super(key: key);

  @override
  State<LoginBodega> createState() => _LoginBodega();
}

class _LoginBodega extends State<LoginBodega> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          await _displayTextInputDialog(context, qr, widget.codBodega!, widget.nombreBodega!);
        },
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) async {
      ...
      if (widget.comprueba == 1) {
        if (qr != scanData.code.toString()) {
          ...
          await guardarSesionDatos(
              int.parse(codUsuario), double.parse(widget.longitud!), double.parse(widget.latitud!), int.parse(widget.codBodega!), token);
          await guardarDatosBodega(widget.codBodega!, widget.nombreBodega!, widget.metodo!);
        ...
        try {
          ...
          if (loginDecode["msg"] == "ok") {
            await guardarSesionDatos(int.parse(codUsuario), double.parse(widget.longitud!), double.parse(widget.latitud!),
                int.parse(loginDecode["data"]["respuesta"][0]["Cod_Bodega"].toString()), token);

            await guardarDatosBodega(loginDecode["data"]["respuesta"][0]["Cod_Bodega"].toString(),
                loginDecode["data"]["respuesta"][0]["Descripcion"].toString(), widget.metodo!);
            await ArtSweetAlert.show(
                barrierDismissible: false,
                context: context,
                artDialogArgs: ArtDialogArgs(
  ...
  Future<void> _displayTextInputDialog(BuildContext context, String qr, String codBodega, String nombreBodega) async {
        ...
        artDialogArgs: ArtDialogArgs(
          ...
          onConfirm: () async {
            if (qr.toLowerCase() == _textFieldController.text.toString().trim().toLowerCase()) {
              await guardarDatosBodega(codBodega, nombreBodega, widget.metodo!);
              await ArtSweetAlert.show(
                  barrierDismissible: false,
                  context: context,
                  artDialogArgs: ArtDialogArgs(
